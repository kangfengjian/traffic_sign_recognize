...
net = torch.nn.Sequential(
    ...
    nn.Conv2d(6,16,kernel_size=5,padding=0),nn.Sigmoid(),  # torch.Size([6, 1, 5, 5])   torch.Size([6])
    nn.AvgPool2d(kernel_size=2,stride=2),
    nn.Flatten(),
    nn.Linear(16*23*23,372),nn.Sigmoid(),   # torch.Size([120, 400])  torch.Size([120])
    nn.Linear(372,124),nn.Sigmoid(),     # torch.Size([84, 120])   torch.Size([84])
    nn.Linear(124,62)            # torch.Size([10, 84])   torch.Size([10])
)
...
def train_ch6(net,train_iter,test_iter,num_epochs,lr,device):
    ...
    for epoch in range(num_epochs):
        ...
        if test_acc>best_acc:
            ...
            torch.save(net.state_dict(),weights_path/Path('LeNet_best__weights.pth'))
        torch.save(net.state_dict(), weights_path/Path('LeNet_last__weights.pth'))
...
def test(test_loader,weights,device):
    ...
    with open('result/LeNet_{}.csv'.format(datetime.now().strftime("%Y-%m-%d_%H:%M:%S")),'w',encoding='utf-8') as wf:
...
if __name__ == '__main__':
    ...
    train_loader = DataLoader(dataset=TrafficSign(datasets_root,data_root,'train'),batch_size=512)
    val_loader = DataLoader(dataset=TrafficSign(datasets_root,data_root,'val'),batch_size=512)
    ...
    test(test_loader,'weights/LeNet_best__weights.pth',d2l.try_gpu())
